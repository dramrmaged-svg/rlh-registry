...
exports.BCLC_FORMULA_VERSION = void 0;
exports.calculateBclcStage = calculateBclcStage;
 ...
 */
exports.BCLC_FORMULA_VERSION = 'LEGACY_BCLC_STAGE_2022_v105';
function isMilanEligible(count, largest, pvtt, ehd) {
    if (ehd || pvtt.includes('main') || pvtt.includes('lobar'))
        return false;
    return count <= 3 && largest <= 5 && !(count === 1 && largest > 5) && !(count > 1 && largest > 3);
}
function calculateBclcStage(input) {
    ...
    if (cp === 'C' || ecog >= 4)
        stage = 'D';
    else if (ehd || pvtt.includes('main') || pvtt.includes('lobar') || (!Number.isNaN(ecog) && ecog >= 2))
        stage = 'C';
    else if ((cp === 'A' || cp === 'B') && count > 3)
        stage = 'B';
    else if ((cp === 'A' || cp === 'B') && !(count === 1 && largest <= 2) && !milan)
        stage = 'B';
    else if ((cp === 'A' || cp === 'B') && ((count === 1 && largest <= 2 && !pvtt) || (milan && cp === 'A')))
        stage = 'A';
    else if (cp === 'A' && count === 1 && largest <= 2 && !pvtt)
        stage = '0';
    if (stage === null) {
        ...
        return (0, types_1.notCalculable)(exports.BCLC_FORMULA_VERSION, sourceFields, missingFields, 'Insufficient data for BCLC 2022 staging — Child-Pugh grade, ECOG, tumour count/size, PVTT and extrahepatic spread status are all required by the decision cascade.');
    }
    return (0, types_1.calculated)(stage, exports.BCLC_FORMULA_VERSION, sourceFields, `BCLC 2022 staging cascade (Reig M et al. J Hepatol 2022;76:681-693) as implemented in the legacy app, stage ${stage}.`);
}
