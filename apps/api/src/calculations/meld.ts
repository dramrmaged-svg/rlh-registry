 ...
 */
export const MELD3_FORMULA_VERSION = 'LEGACY_CALC_MELD3_v105';
export const MELD_NA_FORMULA_VERSION = 'LEGACY_CALC_MELD_NA_v105_DOUBLE_SODIUM_ADJUSTMENT_UNVERIFIED';
...
export interface Meld3Input {
  ...
  sex?: 'MALE' | 'FEMALE' | 'INDETERMINATE' | 'UNKNOWN' | null;
  /**
   * Legacy only checked `dialysis === "Yes"` (a binary flag) to force the
   * creatinine component to its ceiling; `dialysisSessionsPastWeek` is
   * captured in the schema for future clinical use but was never consumed
   * by the legacy formula and is not used here either.
   */
...
function meld3Inputs(input: Meld3Input): { bili: number; alb: number; inrV: number; cr: number; na: number } | null {
  const { bilirubinUmolL: bili, albuminGL: alb, inr: inrV, creatinineUmolL: cr, sodiumMmolL: na } = input;
  const values = [bili, alb, inrV, cr, na];
  if (values.some((v) => v === null || v === undefined || !Number.isFinite(v) || (v as number) <= 0)) return null;
  return { bili: bili as number, alb: alb as number, inrV: inrV as number, cr: cr as number, na: na as number };
}

function computeMeld3Raw(v: { bili: number; alb: number; inrV: number; cr: number; na: number }, sex: Meld3Input['sex'], onDialysis: boolean | null | undefined): number {
...
export function calculateMeld3(input: Meld3Input): CalculationResult<number> {
  const v = meld3Inputs(input);
  if (!v) {
    return notCalculable(MELD3_FORMULA_VERSION, MELD3_SOURCE_FIELDS, ['bilirubinUmolL', 'sodiumMmolL', 'inr', 'creatinineUmolL', 'albuminGL'].filter((f) => {
      const val = (input as unknown as Record<string, number | null | undefined>)[f];
  ...
  return calculated(m3, MELD3_FORMULA_VERSION, MELD3_SOURCE_FIELDS, 'MELD-3.0 (FMP formula), clamped to [6, 40]. See source comments for the unset-sex and dialysis handling ported from the legacy app.');
}

export function calculateMeldNa(input: Meld3Input): CalculationResult<number> {
  const v = meld3Inputs(input);
  if (!v) {
    return notCalculable(MELD_NA_FORMULA_VERSION, MELD3_SOURCE_FIELDS, ['bilirubinUmolL', 'sodiumMmolL', 'inr', 'creatinineUmolL', 'albuminGL'].filter((f) => {
      const val = (input as unknown as Record<string, number | null | undefined>)[f];
      return val === null || val === undefined || !Number.isFinite(val) || val <= 0;
  ...
  return calculated(
    mn,
    MELD_NA_FORMULA_VERSION,
    MELD3_SOURCE_FIELDS,
    'MELD-Na = MELD-3.0 with the classic 2016 sodium correction applied on top — ported faithfully from the legacy app; NOT clinically verified, since MELD-3.0 is already sodium-adjusted (see module doc comment).',
  );
}
